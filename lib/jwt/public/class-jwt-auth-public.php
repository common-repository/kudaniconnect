...
class Jwt_Auth_Public
{
    ...
    {
        $secret_key = wp_salt(); //defined('JWT_AUTH_SECRET_KEY') ? JWT_AUTH_SECRET_KEY : false;
        ...
        /** Store token **/
        update_option( 'kudani_jwt_secret_key', $token );
    ...
    public function determine_current_user($user)
    {
         ...
         * @since 1.2.3
         **/
         //simion314 HACK to get both kind of rest endpoint to work
        $rest_api_slug = "?rest_route=";//rest_get_url_prefix();
        $valid_api_uri1 = strpos($_SERVER['REQUEST_URI'], $rest_api_slug) ;
        $rest_api_slug = "?rest_route=";//rest_get_url_prefix();
        $valid_api_uri2 = strpos($_SERVER['REQUEST_URI'], $rest_api_slug) ;
        $valid_api_uri = $valid_api_uri1 || $valid_api_uri2;
        if(!$valid_api_uri){
            //throw new Exception("e11 " ." $rest_api_slug " . $_SERVER['REQUEST_URI']);
            return $user;
        }
        ...
        if (is_wp_error($token)) {
            if ($token->get_error_code() != 'jwt_auth_no_auth_header') {
                ...
                //throw new Exception("e2 " . print_r($token, true));
                return $user;
            } else {
                return $user;
            }
    ...
    {
        /*
         * Looking for the HTTP_COOKIE header, if not present just
         * return the user.
         */
        ...
        $saved_token = get_option( 'kudani_jwt_secret_key' );
        if ($saved_token != $auth) {
            return new WP_Error(
                'jwt_auth_incorrect_token',
                __('Authorization token incorrect.', 'wp-api-jwt-auth'),
                array(
                    'status' => 403,
                )
            );
